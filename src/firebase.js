...
function newKey(path) {
  return window.firebase.database().ref().child(path).push().key;
}

// TODO: Handle errors
export function* get(path, key) {
  const ref = window.firebase.database().ref(`${path}/${key}`);
...
export function* sync(path, mapEventToAction = {}) {
  const ref = window.firebase.database().ref(path).limitToLast(20);
